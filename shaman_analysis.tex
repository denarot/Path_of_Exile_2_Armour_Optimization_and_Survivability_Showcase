...
\begin{enumerate}
    \item \textbf{Armour is inefficient against large hits} --- the DR formula's denominator grows linearly with hit size, collapsing DR\% exactly when it matters most.
    \item \textbf{eHP is the binding constraint} --- the minimum life pool required to survive a given hit is dictated by what the formula allows through, and reducing that requirement via conversion is categorically more efficient than raising armour to compensate.
...
\subsection*{Build Parameters for the Comparison}
...
\namedsubsection{Why Both Look Comparable at Small Hits}
...
\namedsubsection{The Break Point: Where Hit Size Threatens the Formula}
...
\namedsubsection{The ``Below 90\%'' Threshold}
