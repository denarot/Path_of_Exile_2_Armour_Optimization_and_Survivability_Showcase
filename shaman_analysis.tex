...
\subsection*{The Armour Optimisation Corridor: Formal Derivation of the 12$\times$--20$\times L$ Planning Constraint}

For any pure-armour build, the minimum armour required to survive a physical hit of size $D$ on a life pool of size $L$ follows directly from the survival condition. Damage taken is:

\[
\text{taken} = D\!\left(1 - \frac{A}{A + 10D}\right) = \frac{10D^2}{A + 10D}
\]

Survival requires taken $\leq L$:

\[
\frac{10D^2}{A + 10D} \leq L
\]

Multiply both sides by $(A + 10D)$ and isolate $A$:

\[
10D^2 \leq L(A + 10D) \quad \Rightarrow \quad LA \geq 10D^2 - 10LD \quad \Rightarrow \quad A \geq \frac{10D(D - L)}{L}
\]

When $D \leq L$ the right-hand side is non-positive --- the hit cannot kill the character regardless of armour. The binding regime is $D > L$.

\textbf{Expressing the threat as a multiple of the life pool.} Write $D = kL$ where $k > 1$ is the threat ratio (how many times the life pool the incoming hit represents). Substituting:

\[
A^* = \frac{10(kL)(kL - L)}{L} = \frac{10kL \cdot L(k-1)}{L} = 10k(k-1) \cdot L
\]

The $L$'s cancel entirely. The minimum required armour is a pure multiple of the life pool, and that multiple depends \textit{only on the threat ratio} $k$, not on the absolute pool size:

\[
\boxed{A^* = 10k(k-1) \cdot L}
\]

Plugging in two reference threat ratios establishes the planning corridor:

\begin{itemize}
    \item \textbf{Soft cap} ($k = 1.7$ --- hit 70\% larger than the pool): $A^* = 10(1.7)(0.7)L = 11.9L \approx 12L$
    \item \textbf{Hard cap} ($k = 2.0$ --- hit exactly twice the pool): $A^* = 10(2.0)(1.0)L = 20L$
\end{itemize}

This is the origin of the $12\times$--$20\times L$ corridor used throughout this paper as the planning constraint for pure-armour archetypes.

\textbf{Caveats.} The anchors $k = 1.7$ and $k = 2.0$ are not derived --- they represent an assumed threat window, the largest untelegraphed hit the build is budgeted to facetank. Because $A^* = 10k(k-1)L$ is quadratic in $k$, the corridor is sensitive to this assumption: moving from $k = 2.0$ to $k = 3.0$ doubles the armour requirement ($A^* = 60L$) while the pool only tripled in threat. The binding empirical unknown is the actual hit-size distribution at a given content tier, which is why the paper's main analysis targets the pessimistic ceiling of $H = 10{,}000$. For conversion builds, substitute the residual physical component $(1 - c) \cdot D$ for $D$ before applying the formula, where $c$ is the total conversion fraction; this collapses the armour requirement by a factor of $(1-c)^2$ relative to an unconverting build at the same pool --- the algebraic origin of why Shaman's 75\% conversion reduces its armour requirement so dramatically.
...
\namedsubsection{The Crossover Spectrum: Armour Investment Across Progression Milestones}

The 100{,}000-armour Titan baseline represents near-maximum endgame investment. At earlier progression stages --- realistic armour values achievable through normal gear and moderate passive-tree commitment --- the entire armour curve shifts left: both the 90\% DR cap boundary and the crossover with 0-armour Shaman occur at proportionally smaller hit sizes.

The crossover follows directly from setting the armour DR equal to Shaman's flat 63.75\%:

\[
\frac{A}{A + 10H^*} = 0.6375 \quad \Rightarrow \quad H^* = \frac{A \times (1 - 0.6375)}{10 \times 0.6375} = \frac{0.3625\, A}{6.375} \approx \frac{A}{17.6}
\]

The crossover scales linearly with armour: every 10{,}000 additional armour rating shifts the crossover rightward by $\approx 569$ hit-size units.

\begin{table}[htbp]
\centering
\caption{\textbf{Crossover Hit Size and 90\% Cap Boundary by Armour Investment Level}}
\begin{tabular}{rrrrp{4.5cm}}
\toprule
\textbf{Armour ($A$)} & \textbf{DR cap below} & \textbf{Crossover $H^*$} & \textbf{Implied $L$ ($A=12L$)} & \textbf{Progression context} \\
\midrule
\rowcolor{rowalt} 25{,}000 & $H \leq 278$ & 1{,}422 & 2{,}083 & Early maps \\
36{,}000 & $H \leq 400$ & 2{,}047 & 3{,}000 & Mid-tier maps \\
\rowcolor{rowalt} 48{,}000 & $H \leq 533$ & 2{,}729 & 4{,}000 & Late maps / early endgame \\
100{,}000 & $H \leq 1{,}111$ & 5{,}686 & 8{,}333 & Near-maximum endgame \\
\bottomrule
\end{tabular}
\smallskip
\begin{minipage}{\linewidth}
\small\textit{Implied $L$ uses the corridor soft cap $A = 12L$. DR cap boundary solves $A/(A+10H)=0.90$; below this threshold, the armour build is at the 90\% cap and the incremental DR advantage over Shaman is at its maximum (26.25 pp). At the soft-cap corridor, $H^* \approx 0.68L$ for every armour level: the crossover consistently falls at 68\% of the life pool, regardless of absolute values.}
\end{minipage}
\end{table}
...
\begin{axis}[
    ...
    xmin=0, xmax=10500,
    ymin=15, ymax=100,
    xtick={0,1000,2000,3000,4000,5000,6000,8000,10000},
    xticklabels={0,1{,}000,2{,}000,3{,}000,4{,}000,5{,}000,6{,}000,8{,}000,10{,}000},
    x tick label style={rotate=30, anchor=east, font=\small},
    ytick={20,30,40,50,63.75,70,80,90,100},
    yticklabels={20,30,40,50,63.8,70,80,90,100},
...
% A=25k -- early maps
\addplot[color=orange!90!black, thick] coordinates {
    (0,90) (278,90) (500,83.3) (1000,71.4) (1422,63.75)
    (2000,55.6) (3000,45.5) (5000,33.3) (8000,23.8) (10000,20.0)
};
\addlegendentry{A=25{,}000 --- early maps ($H^*$=1{,}422)}

% A=36k -- mid maps
\addplot[color=green!70!black, thick] coordinates {
    (0,90) (400,90) (500,87.8) (1000,78.3) (2000,64.3)
    (2047,63.75) (3000,54.5) (5000,41.9) (8000,31.0) (10000,26.5)
};
\addlegendentry{A=36{,}000 --- mid maps ($H^*$=2{,}047)}

% A=48k -- late maps
\addplot[color=teal, thick] coordinates {
    (0,90) (533,90) (1000,82.8) (2000,70.6) (2729,63.75)
    (3000,61.5) (5000,49.0) (8000,37.5) (10000,32.4)
};
\addlegendentry{A=48{,}000 --- late maps ($H^*$=2{,}729)}

% A=100k -- near-max endgame
\addplot[color=red!80!black, thick] coordinates {
    (0,90) (1111,90) (2000,83.3) (4000,71.4) (5686,63.75)
    (8000,55.6) (10000,50.0)
};
\addlegendentry{A=100{,}000 --- near-max endgame ($H^*$=5{,}686)}

% Shaman 0-arm (flat)
\addplot[color=blue!80!black, thick, dashed] coordinates {
    (0,63.75) (10000,63.75)
};
\addlegendentry{Shaman 0-armour --- flat 63.75\%}

% 90% cap reference
\addplot[color=gray!60, dashed, thin] coordinates {(0,90) (10500,90)};

% Vertical dashed lines at each crossover
\addplot[color=orange!70!black, dashed, thin] coordinates {(1422,15) (1422,63.75)};
\addplot[color=green!60!black,  dashed, thin] coordinates {(2047,15) (2047,63.75)};
\addplot[color=teal!80!black,   dashed, thin] coordinates {(2729,15) (2729,63.75)};
\addplot[color=red!60!black,    dashed, thin] coordinates {(5686,15) (5686,63.75)};

% Crossover labels on x-axis area
\node[anchor=north, font=\scriptsize, rotate=90, color=orange!90!black] at (axis cs:1422,39) {$H^*_{25\text{k}}$=1{,}422};
\node[anchor=north, font=\scriptsize, rotate=90, color=green!70!black]  at (axis cs:2047,39) {$H^*_{36\text{k}}$=2{,}047};
\node[anchor=north, font=\scriptsize, rotate=90, color=teal]            at (axis cs:2729,39) {$H^*_{48\text{k}}$=2{,}729};
\node[anchor=north, font=\scriptsize, rotate=90, color=red!80!black]    at (axis cs:5686,39) {$H^*_{100\text{k}}$=5{,}686};

\node[anchor=west, font=\footnotesize, color=gray]          at (axis cs:200,91.5) {90\% DR cap};
\node[anchor=west, font=\footnotesize, color=blue!80!black] at (axis cs:6800,65.2) {63.75\%};

\end{axis}
\end{tikzpicture}
\caption{\textbf{Physical DR vs.\ Hit Size at Four Armour Investment Levels.} Each armour curve is horizontal at the 90\% DR cap for small hits, then declines. The crossover with Shaman's flat 63.75\% line (blue dashed) shifts leftward with lower armour investment: A=25{,}000 (orange, early maps) crosses at $H=1{,}422$; A=36{,}000 (green, mid maps) at $H=2{,}047$; A=48{,}000 (teal, late maps) at $H=2{,}729$; A=100{,}000 (red, near-maximum endgame) at $H=5{,}686$. To the left of each crossover the armour build has superior DR\%; to the right, 0-armour Shaman is superior. The three lower curves represent realistic progression-stage armour values, demonstrating that 100{,}000-armour Titan is the outlier, not the representative case.}
\label{fig:crossover_spectrum}
\end{figure}

The figure makes two points simultaneously. First, every armour investment level eventually loses to 0-armour Shaman above its crossover threshold --- there is no armour value that maintains DR superiority at the endgame hit ceiling. Second, the threshold moves substantially with investment: at early-map armour values (25{,}000), the crossover arrives at $H = 1{,}422$; to push it to $H = 5{,}686$ requires four times the armour and a correspondingly larger life pool under the corridor constraint.

The practical implication is that the armour build's DR\% advantage is confined to a regime --- below each $H^*$ --- where neither build can be one-shot on a life pool at the corridor minimum. At the soft-cap corridor ($A = 12L$), $H^* \approx 0.68L$: the crossover is always below the pool size. A hit that is 68\% of your life pool is dangerous but not a one-shot threat. The build choice that determines \textit{one-shot survival} is therefore always made in the regime where 0-armour Shaman already has the DR\% advantage. The armour build's advantage in the sub-crossover regime is real --- it produces a more stable health globe and better quality-of-life under continuous moderate incoming damage --- but it is a comfort advantage, not a survival-ceiling advantage.
...
The comparative analysis in Section 3 treated Smith of Kitava as a pure-armour archetype with no elemental handling layer. A structurally superior configuration adds two elements: passive investment in ``X\% of Armour applies to Elemental Damage'' nodes and a 35\% physical-to-elemental conversion source. The configuration substantially reduces the armour planning constraint. With 35\% conversion, the physical residual entering the formula shrinks from $H$ to $0.65H$; the elemental handling layer routes the remaining 35\% through armour DR rather than resistance alone. The minimum armour to survive the endgame ceiling falls to 5.0--10.5$\times L$ depending on $Y$-factor and resistance configuration --- against the Titan's 20$\times L$ hard-cap corridor derived in Section~1. The conversion changes not just what each armour point accomplishes, but how many armour points the build must commit.
...
The complete endgame Smith configuration --- armour within the conversion-adjusted demand range (quantified in the following subsection), $Y$-stacking to reach 75\%+ elemental DR, Veil of Night as a pure life multiplier, Blade Catcher providing crit immunity and $2A$ effective defence on crits --- achieves a defensive profile that is inferior to Shaman in general endgame content but that dominates the critical strike map frontier. In that content specifically, the hits that would one-shot other archetypes land at base damage against double effective armour, and the elemental components of those crits are handled by the armour formula rather than by resistances. Smith occupies a narrower but genuine niche: the armour-based specialist for content where critical strikes are the primary threat.

\namedsubsection{Conversion-Adjusted Armour Demand: The SoK Corridor vs Titan}

For the Titan with no conversion, the minimum armour to survive a hit $D = 2L$ on pool $L$ is $A^* = 20L$ (Section~1). With 35\% conversion, the physical residual entering the armour formula is $0.65H$ and the elemental component is handled by the armour-to-elemental layer, so the total survival condition is:

\[
\underbrace{\frac{4{,}225 \cdot H^2}{A + 6{,}500}}_{\text{physical taken}} + \underbrace{\frac{1{,}225 \cdot H^2 \cdot (1-r)}{Y \cdot A + 3{,}500}}_{\text{elemental taken}} \leq L
\]

No closed form isolates $A$ in the combined system, but numerical solution at $H = 10{,}000$, $L = 5{,}000$ yields:

\begin{table}[htbp]
\centering
\caption{\textbf{Minimum Armour to Survive H=10{,}000 on L=5{,}000 --- SoK (35\% Conversion) vs Titan}}
\begin{tabular}{llrrc}
\toprule
\textbf{Configuration} & \textbf{Elemental coverage} & \textbf{$A^*$ (min.)} & \textbf{$A^*$ as $\times L$} & \textbf{vs Titan} \\
\midrule
\rowcolor{rowalt} Titan (no conversion, $r=75$\%) & N/A (pure armour) & 100{,}000 & 20.0$\times$ & baseline \\
SoK $Y=1.0$, Veil of Night ($r=0$\%) & Armour DR only & 52{,}500 & 10.5$\times$ & $-$47{,}500 \\
\rowcolor{rowalt} SoK $Y=2.0$, Veil of Night ($r=0$\%) & Armour DR only & 41{,}500 & 8.3$\times$ & $-$58{,}500 \\
SoK $Y=3.0$, Veil of Night ($r=0$\%) & Armour DR only & 37{,}000 & 7.4$\times$ & $-$63{,}000 \\
\rowcolor{rowalt} SoK $Y=1.0$, standard ($r=75$\%) & Armour DR + resistance & 29{,}000 & 5.8$\times$ & $-$71{,}000 \\
SoK $Y=2.0$, standard ($r=75$\%) & Armour DR + resistance & 26{,}000 & 5.2$\times$ & $-$74{,}000 \\
\rowcolor{rowhi} SoK $Y=3.0$, standard ($r=75$\%) & Armour DR + resistance & 25{,}000 & 5.0$\times$ & $-$75{,}000 \\
\bottomrule
\end{tabular}
\smallskip
\begin{minipage}{\linewidth}
\small\textit{$A^*$ is the minimum armour at which total damage taken equals exactly $L = 5{,}000$ (0 HP remaining after the hit). Practical builds target $A$ well above $A^*$ for survival margin. With standard resistance ($r = 75$\%): $Y$-stacking above $Y = 1.0$ provides modest additional reduction (5.8$\times$ $\to$ 5.0$\times$) because 75\% resistance already handles most of the elemental component. With Veil of Night ($r = 0$\%): $Y$-stacking is load-bearing (10.5$\times$ $\to$ 7.4$\times$), as armour DR is the only tool for the elemental component. In both regimes, SoK's armour demand is dramatically lower than the Titan's 20$\times L$ floor.}
\end{minipage}
\end{table}

Smith of Kitava's build identity follows directly: \textit{armour efficient enough at lower investment, freeing budget for life, damage, and additional defensive layers}. At $L = 5{,}000$, the full-investment build running 80{,}000 armour operates at roughly $3$--$4\times A^*$ above the survival minimum, giving substantial margin without the Titan's full 100{,}000 armour commitment.

\namedsubsection{Y-Factor Mitigation at Standard Investment ($A = 80{,}000$)}

At $A = 80{,}000$ (upper end of the conversion-adjusted corridor for $L = 5{,}000$), physical damage taken is fixed across all configurations:

\[
\text{DR}_{\text{phys}} = \frac{80{,}000}{145{,}000} = 55.2\% \quad \Rightarrow \quad 6{,}500 \times 0.448 = 2{,}912 \text{ physical taken (all configs)}
\]

The $Y$-factor affects only the elemental component, with distinct impact profiles depending on whether Veil of Night is active:

\begin{table}[htbp]
\centering
\caption{\textbf{SoK Mitigation by $Y$-Factor and Resistance Configuration ($A = 80{,}000$, $H = 10{,}000$)}}
\begin{tabular}{llrrrr}
\toprule
\textbf{$Y$} & \textbf{Resistance} & \textbf{Ele DR} & \textbf{Ele Taken} & \textbf{Total Taken} & \textbf{Mitigation} \\
\midrule
\rowcolor{rowalt} 1.0 & 75\% (standard) & 69.6\% & 266 & 3{,}178 & 68.2\% \\
2.0 & 75\% (standard) & 82.1\% & 157 & 3{,}069 & 69.3\% \\
\rowcolor{rowalt} 3.0 & 75\% (standard) & 87.3\% & 111 & 3{,}023 & 69.7\% \\
\midrule
1.0 & 0\% (Veil of Night) & 69.6\% & 1{,}065 & 3{,}977 & 60.2\% \\
\rowcolor{rowalt} 2.0 & 0\% (Veil of Night) & 82.1\% & 628 & 3{,}540 & 64.6\% \\
3.0 & 0\% (Veil of Night) & 87.3\% & 445 & 3{,}357 & 66.4\% \\
\midrule
\rowcolor{rowhi} Reference: Titan ($Y=0$, $r=75$\%) & 75\% & 44.4\% & 0 (no ele) & 5{,}556 & 44.4\% \\
\bottomrule
\end{tabular}
\smallskip
\begin{minipage}{\linewidth}
\small\textit{Physical taken = 2{,}912 across all SoK rows (fixed by armour and physical component). Ele Taken = $3{,}500 \times (1 - \text{Ele DR}) \times (1-r)$; all SoK rows use 35\% conversion. \textbf{Standard resistance:} $Y$-stacking above $Y = 1.0$ yields marginal mitigation gains (68.2\% $\to$ 69.7\%) because 75\% resistance already suppresses the elemental component; the elemental taken at $Y = 1.0$ is only 266 of the 1{,}065 pre-resistance value. \textbf{Veil of Night:} $Y$-stacking is the primary mitigation tool for the elemental component --- each $Y$ step delivers a material improvement (60.2\% $\to$ 64.6\% $\to$ 66.4\%). At $Y = 3.0$ with VoN, elemental DR reaches 87.3\% --- within the range that 75\% resistance alone would provide, confirming the theoretical claim that high-$Y$ configurations make Veil of Night's 0\% resistance cost-free. Titan row confirms SoK at any configuration outperforms pure armour by 14--25 percentage points at equal armour investment.}
\end{minipage}
\end{table}

\namedsubsection{Smith's Masterwork: Ascendancy Allocation and Build Philosophy}

Smith of Kitava's ascendancy tree is structured around a central defensive cluster, \textbf{Smith's Masterwork}, that converts passive skill point investment into armour at 200 armour per point allocated. This is the mechanism by which the build reaches the 40{,}000--80{,}000 armour range without exhausting gear affixes: passive tree points directed through armour clusters each contribute both the node's listed bonus and the Masterwork conversion rate on top. At 50 passive points in armour nodes, the Masterwork contribution alone adds 10{,}000 armour on top of geared values.

\textbf{The full Masterwork ascendancy path} consumes all eight available ascendancy points across the following nodes:

\begin{enumerate}
    \item Dedication to Kitava
    \item Internal Layer
    \item Leather Bindings
    \item Molten Symbol
    \item Flowing Metal
    \item Heatproofing
    \item Support Straps
    \item Kitavan Engravings
\end{enumerate}

\textbf{Substitution options.} Leather Bindings and Flowing Metal can each be replaced by \textbf{Tribute to Utula} or \textbf{Kitavan Imprint} if the build requires what those nodes provide rather than what the standard pair delivers. These swaps consume the same two ascendancy point pairs and do not change the armour-per-point Masterwork rate; the decision is based on what the specific build needs from those two slots.

\textbf{Recommended investment tiers.}

\begin{itemize}
    \item \textbf{Full Masterwork (``full coward's ascendancy'').} All eight ascendancy points in the defensive cluster. Fully activates the Masterwork armour-per-point conversion and all layer interactions (Internal Layer physical reduction, Blade Catcher crit conversion, armour-to-elemental stacking). The passive tree is then free to pursue a damage--defence mix, since the ascendancy handles the defensive ceiling without further support from the ascendancy budget. \textit{Recommended for players targeting the endgame armour corridor and prioritising deterministic survival.}

    \item \textbf{Pseudo-complete (6 of 8 points, last two deferred).} The final two ascendancy point pairs require completing the hardest available Trials content, which may not be accessible during active campaign or early endgame progression. Taking the first six nodes activates the majority of the Masterwork ceiling. Regular passive skill points compensate defensively in the interim --- an additional 8--15 passive points in armour and life clusters covers the gap --- while Trial completion is pursued over time. \textit{Recommended for players in active progression who have not yet cleared the relevant Trials.}
\end{itemize}

\textbf{Passive tree allocation alongside full Masterwork.} The Masterwork conversion rate makes generic armour-cluster passive nodes more efficient on Smith than on any other archetype: each point contributes the node's listed bonus \textit{plus} the 200-armour Masterwork rate. A balanced allocation for the passive tree after filling the ascendancy:

\begin{itemize}
    \item \textbf{8--15 nodes:} armour clusters (reaching 40{,}000--80{,}000 total armour with Masterwork + gear, placing the build at 8--16$\times L$ above the minimum survival threshold for the VoN configuration)
    \item \textbf{Heavy investment:} life nodes, targeting $L \geq 4{,}000$--$5{,}000$ (each additional 500 HP raises the armour corridor ceiling and widens the survival margin directly)
    \item \textbf{Remaining budget:} split between damage scaling and additional defensive layers (evasion, maximum resistance upgrades)
\end{itemize}

The build's asymmetry is that the ascendancy handles the defensive ceiling through the Masterwork rate, freeing the passive tree for a genuine offensive--defensive tradeoff rather than forcing full commitment to armour clusters to reach the survival threshold.
