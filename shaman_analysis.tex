...
\subsection*{Optimal Smith Configuration: Armour-to-Elemental and Veil of Night}

The analysis above treats Smith of Kitava as a pure-armour archetype with no elemental handling layer. At endgame investment, a structurally superior configuration emerges by combining three elements: passive investment in ``X\% of Armour applies to Elemental Damage'' nodes, a 35\% physical-to-elemental conversion source, and \textit{Veil of Night} as the chest slot.

With 35\% conversion, the incoming hit splits into a physical component of $0.65H$ and an elemental component of $0.35H$. The physical component evaluates against armour normally; the elemental component evaluates against armour scaled by the armour-to-elemental percentage $Y$ (as established in the mathematical framework, Section 2):

\[
\text{DR}_{\text{phys}} = \frac{A}{A + 6.5H} \qquad \text{DR}_{\text{ele}} = \frac{Y \cdot A}{Y \cdot A + 3.5H}
\]

At $Y = 1.0$ (full armour applies to elemental) and $A = 200{,}000$, both components achieve high DR against a 10{,}000 raw physical hit:

\begin{align*}
\text{Physical DR:} \quad &\frac{200{,}000}{265{,}000} = 75.5\% \quad \Rightarrow \quad 6{,}500 \times 0.245 = 1{,}593 \text{ damage} \\
\text{Elemental DR:} \quad &\frac{200{,}000}{235{,}000} = 85.1\% \quad \Rightarrow \quad 3{,}500 \times 0.149 = 522 \text{ damage} \\
\text{Total damage taken:} \quad &1{,}593 + 522 = 2{,}115 \quad (78.9\%\text{ total mitigation})
\end{align*}

This approaches Shaman's 80.8\% at 25{,}000 armour --- at 8$\times$ the armour investment. The divergence analysis in the preceding subsections continues to apply at all intermediate armour values; the structural Smith weakness against hits above H$\approx$1{,}111 is unchanged below 100{,}000 armour. At the endgame ceiling, however, the armour-to-elemental configuration meaningfully closes the gap.

\textbf{Veil of Night as the enabler.} Once armour-to-elemental scaling is in place, elemental resistances become redundant: the armour formula handles the converted elemental component directly, and the 0\% elemental resistance imposed by Veil of Night has no cost. The 50\% increased maximum life is pure gain --- a 3{,}500 base life pool becomes 5{,}250, which trivially absorbs the 2{,}115 damage computed above while freeing every elemental resistance affix on the character's gear for additional armour, life, or offensive investment.

The minimum armour threshold for 75\% DR on the elemental component at H=10{,}000 is approximately 105{,}000 armour (derived from $A / (A + 35{,}000) = 0.75$). This is achievable at high gear investment and is the correct trigger point for equipping Veil of Night. Below this threshold, the 0\% elemental resistance leaves the elemental component unmitigated and Veil of Night is self-destructive. The build order is fixed: acquire armour-to-elemental passive nodes and reach the 105k armour threshold first; Veil of Night is the last major piece, not an early investment.
...
\smallskip
\begin{minipage}{\linewidth}
\small\textit{The constant-reduction comparison is unambiguous: Wooden Wall routes 20\% of every hit to the totem while Jade Heritage reduces only 10\% terminally. Jade Heritage's Guard skill provides a conditional absorptive buffer equal to 60\% of maximum life on a cooldown, which can be activated before a known hit --- but this is a reactive tool with a recharge period, not continuous damage reduction. In sustained encounters with reliable totem uptime, Wooden Wall is the superior defensive choice. One unresolved interaction: it is unclear whether Turtle Charm's 80\% less on blocked hits applies when the player is in the ``Encased in Jade'' state from Jade Heritage; if it does, the path comparison shifts marginally toward Jade Heritage for players who maintain the buff. The 10\% vs.\ 20\% reduction differential does not change regardless.}
\end{minipage}
\end{table}
...
\textbf{Jade Heritage as the alternative.} Players who prefer an active defensive mechanic without totem dependency may choose Jade Heritage. Guard activates a temporary absorptive shield equal to 60\% of maximum life ahead of the life pool; at $L = 4{,}500$: Guard $= 2{,}700$, a meaningful conditional buffer against a failed skill check. The 10\% less terminal multiplier applies to all damage regardless of block outcome. Jade Heritage is the correct choice when consistent totem uptime cannot be maintained or when the player prioritises the Guard skill's on-demand absorptive properties over Wooden Wall's continuous reduction.
